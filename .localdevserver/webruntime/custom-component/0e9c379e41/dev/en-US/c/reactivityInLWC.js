Webruntime.define('lwc/reactivityInLWC', ['lwc', 'lightning/configProvider'], function (lwc, configProvider) { 'use strict';
    ...
    * */
    ...
    class PrimitiveResizeHandler extends lwc.LightningElement {
      ...
      onStart(event, startX) {
        ...
        this.dispatchResizeStartEvent(); // eslint-disable-next-line @lwc/lwc/no-async-operation
      ...
      onEnd(event) {
        this.touchingResizer = false; // Resize happens in the opposite direction in RTL
    ...
    if (_implicitStylesheets$9) {
      tmpl$S.stylesheets.push.apply(tmpl$S.stylesheets, _implicitStylesheets$9);
    }
    tmpl$S.stylesheetTokens = {
    ...
    var cellWithStandardLayout = lwc.registerTemplate(tmpl$X);
    tmpl$X.stylesheets = [];
    tmpl$X.stylesheetTokens = {
    ...
    var bareCustomCell = lwc.registerTemplate(tmpl$Y);
    tmpl$Y.stylesheets = [];
    tmpl$Y.stylesheetTokens = {
    ...
    if (_implicitStylesheets$a) {
      tmpl$Z.stylesheets.push.apply(tmpl$Z.stylesheets, _implicitStylesheets$a);
    }
    tmpl$Z.stylesheetTokens = {
    ...
    class AutoWidthStrategy {
      ...
      getAdjustedColumnWidths(datatableInterface, columnDefs, recomputeAutoWidthRatios) {
        ...
        let columnWidths = []; // if the lengths dont match return
        ...
        let columnWidthsSum = getTotalColumnWidth(columnWidths); // second pass - there could be excess width remaining due to clamping to maxWidth
        ...
        if (tableScrollWidth > 0) {
          ...
          if (dataCellWidths.length === 0) {
            if (headerCellWidths.length > 0) {
              ...
              const totalFlexibleWidth = totalHeaderWidth - totalFixedWidth - totalResizedWidth; // calculate ratio from header cells
            ...
            const totalFlexibleWidth = Math.min(tableScrollWidth, totalCellWidth) - totalFixedWidth - totalResizedWidth; // calculate ratio from data cells
      ...
      _redistributeExtraWidth(expectedTableWidth, columnWidths, columnDefs) {
        ...
        let extraWidthPerColumn = 0; // first distribute space to columns with min width or threshold of min width
        ...
        const totalFixedWidthColumns = totalResizedColumns + totalFixedColumns; // now distribute to every column possible excluding columns with defined widths
      ...
      _redistributeDeficitWidth(expectedTableWidth, columnWidths, columnDefs) {
        ...
        let extraSpace = expectedTableWidth - columnWidthsSum;
        let totalColsToDistribute = 0;
        let extraWidthPerColumn = 0; // first take away width from columns with max width or threshold of max width
        ...
        extraSpace = expectedTableWidth - columnWidthsSum;
        const totalFixedWidthColumns = totalResizedColumns + totalFixedColumns; // now from every column possible excluding columns with defined widths
    ...
    function getAdjustedRowNumberColumnWidth(state) {
      ...
      return Math.max(CHAR_WIDTH * numberOfChars + ROWNUMBER_PADDING
      /* padding */
      + TOOLTIP_ALLOWANCE
      /* primitive-tootlip */
    ...
    function updateSelectedOptionInHeaderActions(state, colKeyValue) {
      ...
      updateWrapTextAndMaxLinesValuesInCells(state, colIndex, colKeyValue); // lets force a refresh on this column, because the wrapText checked value changed.
    ...
    function isActiveCellAnExitCell(state, direction) {
      ...
      rowIndex === -1 && colIndex === 0 && nextRowIndex !== -1 && nextColIndex !== 0 || rowIndex !== -1 && nextRowIndex === -1 && nextColIndex === 0) {
        return true;
      }
    ...
    const handleDatatableLosedFocus = function (event) {
      ...
      !event.relatedTarget || event.relatedTarget && !event.currentTarget.contains(event.relatedTarget) || event.relatedTarget && event.currentTarget.contains(event.relatedTarget) && state.isExiting) {
    ...
    class LightningDatatable extends lwc.LightningElement {
      ...
      set data(value) {
        ...
        this._columnWidthManager.handleDataChange(previousData, data, columns); // set data in state


        setData(this.state, value); // do necessary updates since rows have changed
      ...
      renderedCallback() {
        ...
        handlePrefetch.call(this, template, state); // customerSelectedRows is only valid till render, after it, the one used should be the one from the state.
      ...
      updateRowsState() {
        ...
        this._columnWidthManager.handleRowNumberOffsetChange(state, widthsData); // update celltofocus next to null if the row still exists after indexes calculation
      ...
      updateColumns(columns) {
        ...
        updateColumnWidthsMetadata(state, widthsData); // set the celltofocus next to null if the column still exists after indexes calculation
      ...
      safariHeaderFix() {
        ...
        if (isSafari) {
          ...
          if (thead) {
            ...
            thead.style.display = 'none'; // eslint-disable-next-line @lwc/lwc/no-async-operation
    ...
    class ReactivityInLWC extends lwc.LightningElement {
      ...
      constructor() {
        super();
        this.columns = ['Property Not tracked', 'Property Tracked'];
        this.noTrackObj = {
          fname: 'Shubham',
          id: 'AAA'
        };
    ...
    lwc.registerDecorators(ReactivityInLWC, {
      fields: ["columns", "noTrackObj", "trackObj", "data"]
    });
